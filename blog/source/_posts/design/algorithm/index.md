...
- 内存连续，适合**较少**的相同类型的数据
- 支持**随机访问**，通过**索引**快速访问数据，时间复杂度为 `O(1)`
- 遍历查找元素的时间复杂度为 `O(n)`
- 插入删除元素的时间复杂度为 `O(n)`
...
- 基于指针或者引用的二叉链式存储法
- 基于数组的顺序存储法，适用于**完全二叉树**。如果节点 X 存储在数组中下标为 i 的位置
  - 左子节点,的下标为 `2 * i` 的位置
  - 右子节点的下标为 `2 * i + 1` 的位置
  - 父节点的下标为 `i/2` 的位置
...
不管操作是插入、删除还是查找，时间复杂度其实都跟**树的高度**成正比，也就是 O(height)。
平衡二叉搜索树的高度接近 logn，所以平均查询时间复杂度 O(logN)，
但 BST 不是一个自平衡的树，最坏情况下会退化成链表，导致时间复杂度为 O(N)。
...
叶子节点通过双向链表进行连接，如果我们要求某个区间的数据。只需要拿区间的起始值，在树中进行查找，
当查找到某个叶子节点之后，再顺着链表往后遍历，直到链表中的结点数据值大于区间的终止值为止。

如何解决这个索引占用太多内存的问题呢？借助时间换空间的思路，把索引存储在硬盘中，而非内存中。
通常内存的访问速度是纳秒级别的，而磁盘访问的速度是毫秒级别的。
读取同样大小的数据，从磁盘中读取花费的时间，是从内存中读取所花费时间的上万倍，甚至几十万倍。

比起内存读写操作，磁盘 IO 操作非常耗时，为了尽量减少磁盘 IO 操作，通过增加每个节点的子节点的数量，来尽量降低树的高度。

通过实现 **m 叉树**来实现 B+ 树。不管是内存中的数据，还是磁盘中的数据，操作系统都是按页（一页大小通常是 4KB，这个值可以通过 getconfig PAGE_SIZE 命令查看）来读取的，一次会读一页的数据。如果要读取的数据量超过一页的大小，就会触发多次 IO 操作。
...
- 一个**完全二叉树**，最常用的存储方式就是**数组**。
- 每一个节点的值都必须**大于等于（大顶堆）/小于等于（小顶堆）**其子树中每个节点的值。

应用场景：

- 优先级队列的实现
- 高性能定时器，基于时间进行排序，只需要一次比较堆顶顶时间即可
- 求中位数，通过维护一个大顶堆和一个小顶堆，大顶堆存储小于等于中位数的数据，小顶堆则存储大于中位数的数据，最后大顶堆的堆顶元素即为中位数。
...
- 稳定性，如果待排序的序列中存在值相等的元素，经过排序之后，**相等元素**之间原有的**先后顺序不变**。
- 是否基于**比较和交换**进行排序

稳定排序算法常用语多字段排序，它可以保证**值相等的元素基于上一次排序的结果进行排序**，值不想等的按照当前的稳定排序算法进行排序。

![常见算法](常见算法.png)

### 快速排序

快排是一种**原地、不稳定**的排序算法，也是使用的就是**分治思想**。
为每次迭代的区间确定一个 **pivot 分区点**，基于 pivot 进行分区处理。
...
它的时间复杂度为 `O(nlogn)`，空间复杂度为 `O(n)`，不是原地排序。

![归并排序思路](归并排序思路.png)

### 桶排序

时间复杂度是线性的 `O(n)`。**适合外部排序**，即数据存储在外部磁盘中，数据量比较大，内存有限，无法将数据全部加载到内存中。

它会对数据进行**分片**，分到**有序的桶**中，再对每个桶中的数据进行排序。

计数排序是一种特殊的桶排序，基于某个维度的**数据范围**进行分桶，适用于**数据范围不大**的场景。
...
在长度为 n 的主串中，检查起始位置分别是 0、1、2...n-m 且长度为 m 的 **n-m+1** 个子串，
看有没有跟长度为 m 的模式串匹配的。它的时间复杂度为 `O(n*m)`。

RK（Rabin-Karp）算法是 BF 算法的优化，引入**特殊设计的哈希算法**，通过比较子串和模式串的哈希值，而不是字符串比较。它的时间复杂度为 `O(n)`。

#### BM 算法

BM（Boyer-Moore）算法，在模式串与主串匹配的过程中，当模式串和子串某个字符不匹配的时候，这个字符被称为**坏字符**，它在模式串中的索引为 `si`，通过滑动模式串来寻找模式串中坏字符的位置 `xi`，然后计算出下次匹配用的子串的位置需要移动的位数 `si - xi`，跳过一些肯定不会匹配的情况。

![坏字符规则](BM-坏字符规则.png)

#### KMP 算法

KMP（Knuth Morris Pratt）算法，试图寻找一种规律：在模式串和主串匹配的过程中，当遇到坏字符后，只需要在**子串的好前缀的后缀子串**中，查找**最长的**那个可以跟**模式串的好前缀的前缀子串**匹配的。假设最长的可匹配的那部分前缀子串是 `{v}`，长度是 `k`，模式串中坏字符的索引为 `j`，主串中坏字符的索引为 `i`。那么就可以把子串的位置一次性往后滑动 `j-k` 位。子串和模式串的匹配过程中，每次遇到坏字符的时候，就把 j 更新为 k，i 不变，然后继续比较。

![KMP 算法](<KMP 算法.png>)

它的空间复杂度为 `O(m)`，m 表示模式串的长度；时间复杂度就是 `O(m+n)`。
